...
if __name__ == "__main__":
    ...
    for fi,f in enumerate(files):
        ...
        for horn in range(0,4): #Cannot calibrate horn 1 (0)
            ...
            meanMod = np.mean(np.mod(np.median(data['MOD'][0,:,horn]),90)*4.)
            ...
            for ch,chPair in enumerate(chanPairs):
                ...
                for chP in chPair:
                    ...
                    volts = data['DATA'][0,:,chP+horn*8]
                    volts_bad = data['DATA'][0,:,chP+horn*8]*1.
                    chErr = data['ERR'][0,:,chP+horn*8]
                    ...
                    for i in range(len(scanStarts)-1):
                        ...
                        if len(xi[isNotSource]) > 4:
                            ...
                            bkgdfit = lastfit
                        volts[scanStarts[i]:scanStarts[i+1]] -=bkgdfit(xi)
                    ...
                    if (np.sqrt(np.median(stds**2)) < noiseLims[ch+(horn-1)*4]):
                        ...
                        if (P1 != None):
                            ...
                            Maps  = Control.Destriper(volts.astype('Float64'),volts.size,pix,npix,Medians=True)
                            ipix = hp.query_disc(nside,hp.ang2vec((90.-0.)*np.pi/180.,0.*np.pi/180.),2.*2.355*beam['X'][0]*np.pi/180.)
                            ...
                            #Maps.m[ipix] = 0.
                            #hp.gnomview(Maps.m,reso=3,norm='hist')
                            #pyplot.show()
                    ...
                    if type(P1) != type(None):
                        ...
                        if chP+horn*4 == 3:
                            ...
                            if np.std(Maps.m[Maps.m != 0]) < 3e-4:
        ...
        if type(P1) != type(None):
            TextFiles.AppendFile('CasA_GaussianFits_PerChannel_All',np.concatenate((np.array([f]),Params,Sums,rFacts,np.array([meanEl]),np.array([meanJd]))))#,np.mean(xpos),np.mean(ypos),np.mean(wxpos),np.mean(wypos))))
    ...
    allmaps[wallmaps != 0] = sallmaps[wallmaps != 0]/wallmaps[wallmaps != 0]
    allmaps[allmaps == 0.] = hp.UNSEEN
    hp.write_map('TauAtest.fits', allmaps)
