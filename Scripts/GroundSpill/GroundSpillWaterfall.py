...
if __name__ == '__main__':
    ...
    for f in files:
        ...
        for horn in range(2):
            ...
            nRingSamples = 3000
            nRingsPerStep = 5
            nRings = mask.size/(nRingSamples*nRingsPerStep)

            #Loop through channels:
            for i in range(nRings-1):
                if i == nRings - 1:
                    hi = tod.size
                    lo = i*nRingSamples*nRingsPerStep
                else:
                    hi = (i+1)*nRingSamples*nRingsPerStep
                    lo = i*nRingSamples*nRingsPerStep

                dataStack = tod[lo:hi,6]
                azStack = az[lo:hi]
                maskStack = mask[lo:hi,6]
                jdmean = np.mean(mjd[lo:hi])


                try:
                    bins,xbins,ebins = Binning.bindata(azStack,dataStack,361)
                    pyplot.plot(bins,',')
                    pyplot.show()
                   
                    #if bins.size == 360:
                        #DataAccess.AppendFile('H313uNom30_GSWaterfall.dat',np.concatenate(([jdmean],bins)))
                except ValueError:
                    continue
                #pyplot.plot(azStack,dataStack,',')
                #pyplot.plot(xbins,bins)
                #pyplot.show()
