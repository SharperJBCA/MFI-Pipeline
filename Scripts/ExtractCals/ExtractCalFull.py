...
if __name__ == '__main__':
    ...
    #Output columns:
    calcols = ['DATA','CAL','JD','EL','AZ']
    caldir = '/nas/scratch/sharper/QUIJOTE/datos/tod/'
    ...
    for f in files:
        ...
        for horn in range(2,3):
            ...
            for ch in channels:
                foremask = (foremap[pix] == 1) & (data['CAL'][0,:,0]==0)

                for i in range(nrings):
                    dat = data['DATA'][0,i*nsampring:(i+1)*nsampring,ch+horn*8]
                    ...
                    if setsize > 4:
                        ...
                        data['DATA'][0,i*nsampring:(i+1)*nsampring,ch+horn*8] -= bkgdfit(set)
                    else:
                        data['DATA'][0,i*nsampring:(i+1)*nsampring,ch+horn*8] -= np.median(dat)
            ...
            calibs = np.where(tau & (data['CAL'][0,:,0]==0))[0]

            if len(calibs) > 60*50*5:
                DataAccess.WriteFits.WriteTable('FullData/'+f,
                                                [data['DATA'][:,calibs,16:24],
                                                 data['CAL'][:,calibs,:],
                                                 data['JD'][:,calibs,:],
                                                 data['EL'][:,calibs,:],
